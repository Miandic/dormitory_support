...
def change_kb(user_telegram_id: int):
    kb_list = []
    kb_list.append([InlineKeyboardButton(text="Главная страница ↩️", callback_data='Home')])
    keyboard = InlineKeyboardMarkup(inline_keyboard=kb_list)
    return keyboard

def link_kb(user_telegram_url: str):
    kb_list = []
    kb_list.append([InlineKeyboardButton(text="Отправитель", url=user_telegram_url)])
    keyboard = InlineKeyboardMarkup(inline_keyboard=kb_list)
    return keyboard
